@extends('pages.dashboard.sidebar', [
    'html_tag_data' => ["override"=>'{ "attributes" : { "placement" : "horizontal", "layout":"fluid" }, "storagePrefix" : "starter-project", "showSettings" : false }'],
    'title' => 'User Details',
    'description'=> 'An empty page with a fluid vertical layout.',
    'breadcrumbs' => ["/Dashboards/passenger"=>"Home"]
])
@section('css')
    <link rel="stylesheet" href="/css/vendor/datatables.min.css"/>

@endsection



@section('js_vendor')
    <script src="/js/cs/scrollspy.js"></script>
    <script src="/js/vendor/jquery.barrating.min.js"></script>
    <script src="/js/vendor/bootstrap-submenu.js"></script>
    <script src="/js/vendor/datatables.min.js"></script>
    <script src="/js/vendor/mousetrap.min.js"></script>
@endsection

@section('js_page')
    <script src="/js/pages/blocks.users.js"></script>
    <script src="/js/cs/datatable.extend.js"></script>
    <script src="/js/plugins/datatable.editablerows.js"></script>
@endsection

@section('page-content')
    <div class="container">
        <div class="row align-items-start">
            <div class="col">

            </div>
            <div class="col">


                <!-- Top Image Start -->
                <section class="scroll-section" id="topImage">
                    <div class="row">
                        <div class="col-auto w-100 sw-md-50">
                            <div class="card mb-3">
                                <div class="card-body mb-n5">
                                    <div class="d-flex align-items-center flex-column mb-5">
                                        <div class="mb-5 d-flex align-items-center flex-column">
                                            <div class="sw-13 position-relative mb-3">
                                                <img src="/img/profile/profile-2.webp" class="img-fluid rounded-xl" alt="thumb" />
                                            </div>
                                            <div class="h5 mb-0">{{$data["passenger"]->first_name}} {{$data["passenger"]->last_name}}</div>
                                            <div class="text-muted">{{$data["passenger"]->profession}}</div>
                                            <div class="text-muted">
                                                <i data-acorn-icon="pin" class="me-1"></i>
                                                <span class="align-middle">{{$data["passenger"]->place_of_birth}}</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="mb-5">
                                        <div class="row g-0 align-items-center mb-2">
                                            <div class="col-auto">
                                                <div class="border border-primary sw-5 sh-5 rounded-xl d-flex justify-content-center align-items-center">
                                                    <i data-acorn-icon="birthday" class="text-primary"></i>
                                                </div>
                                            </div>
                                            <div class="col ps-3">
                                                <div class="row g-0">
                                                    <div class="col">
                                                        <div class="sh-5 d-flex align-items-center lh-1-25">Date Of Birth </div>
                                                    </div>
                                                    <div class="col-auto">
                                                        <div class="sh-5 d-flex align-items-center">{{$data["passenger"]->dob}}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row g-0 align-items-center mb-2">
                                            <div class="col-auto">
                                                <div class="border border-primary sw-5 sh-5 rounded-xl d-flex justify-content-center align-items-center">
                                                    <i data-acorn-icon="building-large" class="text-primary"></i>
                                                </div>
                                            </div>
                                            <div class="col ps-3">
                                                <div class="row g-0">
                                                    <div class="col">
                                                        <div class="sh-5 d-flex align-items-center lh-1-25">Country of Residency</div>
                                                    </div>
                                                    <div class="col-auto">
                                                        <div class="sh-5 d-flex align-items-center">{{$data["passenger"]->country_of_residency}}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row g-0 align-items-center mb-2">
                                            <div class="col-auto">
                                                <div class="border border-primary sw-5 sh-5 rounded-xl d-flex justify-content-center align-items-center">
                                                    <i data-acorn-icon="pen" class="text-primary"></i>
                                                </div>
                                            </div>
                                            <div class="col ps-3">
                                                <div class="row g-0">
                                                    <div class="col">
                                                        <div class="sh-5 d-flex align-items-center lh-1-25">Passport No </div>
                                                    </div>
                                                    <div class="col-auto">
                                                        <div class="sh-5 d-flex align-items-center">{{$data["passenger"]->passport_no}}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row g-0 align-items-center mb-2">
                                            <div class="col-auto">
                                                <div class="border border-primary sw-5 sh-5 rounded-xl d-flex justify-content-center align-items-center">
                                                    <i data-acorn-icon="brush" class="text-primary"></i>
                                                </div>
                                            </div>
                                            <div class="col ps-3">
                                                <div class="row g-0">
                                                    <div class="col">
                                                        <div class="sh-5 d-flex align-items-center lh-1-25">Issue Date </div>
                                                    </div>
                                                    <div class="col-auto">
                                                        <div class="sh-5 d-flex align-items-center">{{$data["passenger"]->issue_date}}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row g-0 align-items-center mb-2">
                                            <div class="col-auto">
                                                <div class="border border-primary sw-5 sh-5 rounded-xl d-flex justify-content-center align-items-center">
                                                    <i data-acorn-icon="collapse" class="text-primary"></i>
                                                </div>
                                            </div>
                                            <div class="col ps-3">
                                                <div class="row g-0">
                                                    <div class="col">
                                                        <div class="sh-5 d-flex align-items-center lh-1-25"> Expiry date </div>
                                                    </div>
                                                    <div class="col-auto">
                                                        <div class="sh-5 d-flex align-items-center">{{$data["passenger"]->expiry_date}}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row g-0 align-items-center mb-2">
                                            <div class="col-auto">
                                                <div class="border border-primary sw-5 sh-5 rounded-xl d-flex justify-content-center align-items-center">
                                                    <i data-acorn-icon="cursor-pointer" class="text-primary"></i>
                                                </div>
                                            </div>
                                            <div class="col ps-3">
                                                <div class="row g-0">
                                                    <div class="col">
                                                        <div class="sh-5 d-flex align-items-center lh-1-25"> Place of issue</div>
                                                    </div>
                                                    <div class="col-auto">
                                                        <div class="sh-5 d-flex align-items-center">{{$data["passenger"]->place_of_issue}}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row g-0 align-items-center mb-2">
                                            <div class="col-auto">
                                                <div class="border border-primary sw-5 sh-5 rounded-xl d-flex justify-content-center align-items-center">
                                                    <i data-acorn-icon="recycle" class="text-primary"></i>
                                                </div>
                                            </div>
                                            <div class="col ps-3">
                                                <div class="row g-0">
                                                    <div class="col">
                                                        <div class="sh-5 d-flex align-items-center lh-1-25"> Arrival date  </div>
                                                    </div>
                                                    <div class="col-auto">
                                                        <div class="sh-5 d-flex align-items-center">{{$data["passenger"]->arrival_date}}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row g-0 align-items-center mb-2">
                                            <div class="col-auto">
                                                <div class="border border-primary sw-5 sh-5 rounded-xl d-flex justify-content-center align-items-center">
                                                    <i data-acorn-icon="shield-check" class="text-primary"></i>
                                                </div>
                                            </div>
                                            <div class="col ps-3">
                                                <div class="row g-0">
                                                    <div class="col">
                                                        <div class="sh-5 d-flex align-items-center lh-1-25"> Visa duration   </div>
                                                    </div>
                                                    <div class="col-auto">
                                                        <div class="sh-5 d-flex align-items-center">{{$data["passenger"]->visa_duration}}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row g-0 align-items-center mb-2">
                                            <div class="col-auto">
                                                <div class="border border-primary sw-5 sh-5 rounded-xl d-flex justify-content-center align-items-center">
                                                    <i data-acorn-icon="star" class="text-primary"></i>
                                                </div>
                                            </div>
                                            <div class="col ps-3">
                                                <div class="row g-0">
                                                    <div class="col">
                                                        <div class="sh-5 d-flex align-items-center lh-1-25">Visa status  </div>
                                                    </div>
                                                    <div class="col-auto">
                                                        <div class="sh-5 d-flex align-items-center">ttt</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </section>
                <!-- Top Image End -->

            </div>
            <div class="col">

            </div>
        </div>
        <div class="d-flex justify-content-center ">
            <!-- Responsive Start -->
            <section class="scroll-section" id="responsive">
                <h2 class="small-title">Passport </h2>
                <div class="row">
                    <div class="col-11">
                        <div class="card mb-5">
                            <div class="card-body">
                                <img src="data:image/jpeg;base64," class="img-fluid rounded" alt="Responsive image" />
                            </div>
                        </div>
                    </div>
                </div>
            </section>

            <!-- Responsive Start -->
            <section class="scroll-section" id="responsive">
                <h2 class="small-title">Passport </h2>
                <div class="row">
                    <div class="col-11">
                        <div class="card mb-5">
                            <div class="card-body">
                                <img src="data:image/jpeg;base64," class="img-fluid rounded" alt="Responsive image" />
                            </div>
                        </div>
                    </div>
                </div>
            </section>

        </div>




        <div class="container">
            <div class="row">
                <div class="col">
                    <!-- Content Start -->
                    <div class="data-table-rows slim">
                        <!-- Table Start -->
                        <div class="data-table-responsive-wrapper">
                            <table id="datatableRows" class="data-table nowrap hover">
                                <thead>
                                <tr>
                                    <th class="text-muted text-small text-uppercase">check in date</th>
                                    <th class="text-muted text-small text-uppercase">check out date</th>
                                    <th class="text-muted text-small text-uppercase">room_type</th>
                                    <th class="text-muted text-small text-uppercase">extra_reservation_id</th>
                                    <th class="empty">&nbsp;</th>
                                    <th class="empty">&nbsp;</th>
                                </tr>
                                </thead>
                                <tbody>
                                @foreach ($data["reservations"] as $reservation)
                                    <tr>
                                        <td><a class="list-item-heading body" href="passengerDetail/{{$reservation->id}}"> {{$reservation->check_in_date}}</a></td>
                                        <td>{{$reservation->check_out_date}}</td>
                                        <td>1</td>
                                        <td>{{$reservation->extra_reservation_id}}</td>
                                        <td></td>
                                        <td></td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
                        </div>
                        <!-- Table End -->
                    </div>
                    <!-- Content End -->

                </div>
            </div>
        </div>

        </div>

@endsection
